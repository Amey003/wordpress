	...
	if ( empty( $posts ) && $members[ 'total' ] == 0 && $groups[ 'total' ] == 0 && !$forums ) {
		...
		<div class="inline-items">
			<div class="author-thumb">
				<img loading="lazy" src="<?php echo get_theme_file_uri( '/images/post-no-image-thumb.png' ); ?>" width="40" height="40" alt="<?php esc_attr_e( 'No image', 'olympus' ); ?>">
			</div>
		...
		foreach ( $post_types as $ptype => $post_type ) {
			...
			foreach ( $post_type as $post ) {
				...
				if ( $img_id	 = get_post_thumbnail_id( $post->ID ) ) {
					...
					$image = '<img loading="lazy" src="' . get_theme_file_uri( '/images/post-no-image-thumb.png' ) . '" width="40" height="40" alt="' . esc_attr__( 'No image', 'olympus' ) . '">';
				}
...
function olympus_add_preloader() {
	...
	if($preloader == 'yes'){
		?>
		<div class="olympus-preloader" data-time="<?php echo $preloader_timeout; ?>">
